...
- In-process rx4 agent loop with scopes, builtins + MCP tools (stdio/http/sse from `~/.telekinesis/mcp.json`), OS sandbox policy, model routing, multi-agent coordination, and secret redaction. Computer-use, skills, and graph memory are compiled with `--features full`.
